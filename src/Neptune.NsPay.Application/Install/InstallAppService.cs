...
using Neptune.NsPay.Migrations.Seed.Host;
...
namespace Neptune.NsPay.Install
{
    [AbpAllowAnonymous]
    [DisableAuditing]
    public class InstallAppService : NsPayAppServiceBase, IInstallAppService
    {
        private readonly AbpZeroDbMigrator<NsPayDbContext> _migrator;
        ...
        {
            var connectionString = _appConfiguration[$"ConnectionStrings:{NsPayConsts.ConnectionStringName}"];
        ...
        {
            EditAppSettingsjson($"ConnectionStrings:{NsPayConsts.ConnectionStringName}", constring);
        }
