...
{
    public class PayOrderEventHandler : IKafkaTopicHandler
    {
        private readonly ISendEndpointProvider _sendEndpointProvider;
        public PayOrderEventHandler(ISendEndpointProvider sendEndpointProvider)
        {
            _sendEndpointProvider = sendEndpointProvider;
        }

        public string Topic => KafkaTopics.PayOrder;
        public async Task HandleAsync(string key, string message)
        {
            ...
            try
            {
                ...
                //推送RabbitMQ
                var payOrder = JsonConvert.DeserializeObject<PayOrderPublishDto>(message);
                var part = MerchantPartitioner.GetPartition(payOrder.MerchantCode);
                ...
                await endpoint.Send(payOrder, ctx =>
                {
