(function () {
    $(function () {
        let _$payOrderDepositsTable = $('#PayOrderDepositsTable');
        let _payOrderDepositsService = abp.services.app.payOrderDeposits;
        ...
        let _selectedDateRang = {
            startDate: moment().startOf('day').add(-1, 'day'),
            endDate: moment().startOf('day').add(2, 'day'),
        };

        let Mindatatime = $("#MinTransactionTimeFilter").daterangepicker({
            timePicker: true,
        ...
        let Maxdatatime = $("#MaxTransactionTimeFilter").daterangepicker({
            timePicker: true,
            timePicker24Hour: true,
            //timePickerSeconds: true,
            singleDatePicker: true,
        ...
        const _associatedOrderModal = new app.ModalManager({
            viewUrl: abp.appPath + 'AppArea/PayOrderDepositsCrypto/AssociatedOrderModal',
            scriptUrl: abp.appPath + 'view-resources/Areas/AppArea/Views/PayOrderDepositsCrypto/_AssociatedOrderModal.js',
            modalClass: 'AssociatedOrderModal'
        });

        const _rejectOrderModal = new app.ModalManager({
            viewUrl: abp.appPath + 'AppArea/PayOrderDepositsCrypto/RejectOrderModal',
            scriptUrl: abp.appPath + 'view-resources/Areas/AppArea/Views/PayOrderDepositsCrypto/_RejectOrderModal.js',
            modalClass: 'RejectOrderModal'
        });

        const _viewPayOrderDepositModal = new app.ModalManager({
            viewUrl: abp.appPath + 'AppArea/PayOrderDepositsCrypto/ViewPayOrderDepositModal',
            modalClass: 'ViewPayOrderDepositsModal'
        });

        const dataTable = _$payOrderDepositsTable.DataTable({
            paging: true,
            serverSide: true,
            processing: true,
            deferRender: true,
            ...
            listAction: {
                ajaxFunction: _payOrderDepositsService.getAllCrypto,
            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    name: "benAccountName",
                    render: function (data, type, full, meta) {
                        var html = "<span>" + app.localize('CryptoWalletAddress') + "\uff1a" + full.payOrderDeposit.accountNo + "</span><br/>";

                        if (full.payOrderDeposit.type == "DBIT")
                            html += "<span>" + app.localize('CryptoWalletAddressTo') + "\uff1a" + full.payOrderDeposit.debitAcctNo + "</span><br/>"
                        else
                            html += "<span>" + app.localize('CryptoWalletAddressTo') + "\uff1a" + full.payOrderDeposit.creditAcctNo + "</span><br/>"
