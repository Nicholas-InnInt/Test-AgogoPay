...
{
    public class PublishNotifyByVcb : BackgroundService
    {
        private readonly ICapPublisher _capBus;
        private readonly IRedisService _redisService;
        private readonly ILogger<PublishNotifyByVcb> _logger;
        ...
        public PublishNotifyByVcb(ILogger<PublishNotifyByVcb> logger,
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            while (!stoppingToken.IsCancellationRequested) {
                _logger.LogInformation("PublishNotifyByVcb Worker running at: {time}", DateTimeOffset.Now);
                ...
                try
                {
                    foreach (var item in merchantConfigs)
                    {
                        ...
                        if (banks != null)
                        {
                            var vcbBankInfo = banks.Where(r => r.IsOpen & r.Type == PayMentTypeEnum.VietcomBank).FirstOrDefault();
                            if (vcbBankInfo != null)
                            {
                                var bankOrderNotifysList = new List<BankOrderNotifyModel>();
                                while (true)
                                {
                                    var bankOrderNotifys = _redisService.GetVcbBankOrderNotifyMqPublish();
                                    ...
                                    if (bankOrderNotifysList.Count >= notifyCount || bankOrderNotifys == null)
                                    {
                                        ...
                                        var bankNotifies = bankOrderNotifysList.Where(r => paygroupInfo.Contains(r.PayMentId) && r.Money >= vcbBankInfo.Money).ToList();
                                        if (bankNotifies.Any())
                                        {
                                            ...
                                            _logger.LogInformation("商户：{merchantcode},Vcb发送mq数据：{data}", item.MerchantCode, bankNotifies.ToJsonString());
                ...
                {
                    _logger.LogError("PublishNotifyByVcb 队列错误：" + ex);
                }
