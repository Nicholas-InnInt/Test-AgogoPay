...
{
    public class PublishNotifyByPVcom : BackgroundService
    {
        private readonly ICapPublisher _capBus;
        private readonly IRedisService _redisService;
        private readonly ILogger<PublishNotifyByPVcom> _logger;
        ...
        public PublishNotifyByPVcom(ILogger<PublishNotifyByPVcom> logger,
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            while (!stoppingToken.IsCancellationRequested) {
                _logger.LogInformation("PublishNotifyByPVcom Worker running at: {time}", DateTimeOffset.Now);
                ...
                try
                {
                    foreach (var item in merchantConfigs)
                    {
                        ...
                        if (banks != null)
                        {
                            var pvComBankInfo = banks.Where(r => r.IsOpen & r.Type == PayMentTypeEnum.PVcomBank).FirstOrDefault();
                            if (pvComBankInfo != null)
                            {
                                var bankOrderNotifysList = new List<BankOrderNotifyModel>();
                                while (true)
                                {
                                    var bankOrderNotifys = _redisService.GetPVcomBankOrderNotifyMqPublish();
                                    ...
                                    if (bankOrderNotifysList.Count >= notifyCount || bankOrderNotifys == null)
                                    {
                                        ...
                                        var bankNotifies = bankOrderNotifysList.Where(r => paygroupInfo.Contains(r.PayMentId) && r.Money >= pvComBankInfo.Money).ToList();
                                        if (bankNotifies.Any())
                                        {
                                            ...
                                            _logger.LogInformation("商户：{merchantcode},PVcom发送mq数据：{data}", item.MerchantCode, bankNotifies.ToJsonString());
                ...
                {
                    _logger.LogError("PublishNotifyByPVcom 队列错误：" + ex);
                }
