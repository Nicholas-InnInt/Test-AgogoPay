...
{
    public class PublishNotifyByMB : BackgroundService
    {
        private readonly ICapPublisher _capBus;
        private readonly IRedisService _redisService;
        private readonly ILogger<PublishNotifyByMB> _logger;
        ...
        public PublishNotifyByMB(ILogger<PublishNotifyByMB> logger,
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            while (!stoppingToken.IsCancellationRequested) {
                _logger.LogInformation("PublishNotifyByMB Worker running at: {time}", DateTimeOffset.Now);
                ...
                try
                {
                    foreach (var item in merchantConfigs)
                    {
                        ...
                        if (banks != null)
                        {
                            var mbBankInfo = banks.Where(r => r.IsOpen & r.Type == PayMentTypeEnum.MBBank).FirstOrDefault();
                            if (mbBankInfo != null)
                            {
                                var bankOrderNotifysList = new List<BankOrderNotifyModel>();
                                while (true)
                                {
                                    var bankOrderNotifys = _redisService.GetMBBankOrderNotifyMqPublish();
                                    ...
                                    if (bankOrderNotifysList.Count >= notifyCount || bankOrderNotifys == null)
                                    {
                                        ...
                                        var bankNotifies = bankOrderNotifysList.Where(r => paygroupInfo.Contains(r.PayMentId) && r.Money >= mbBankInfo.Money).ToList();
                                        if (bankNotifies.Any())
                                        {
                                            ...
                                            _logger.LogInformation("商户：{merchantcode},mb发送mq数据：{data}", item.MerchantCode, bankNotifies.ToJsonString());
                ...
                {
                    _logger.LogError("PublishNotifyByMB 队列错误：" + ex);
                }
