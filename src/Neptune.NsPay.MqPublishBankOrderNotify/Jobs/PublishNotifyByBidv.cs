...
{
    public class PublishNotifyByBidv : BackgroundService
    {
        private readonly ICapPublisher _capBus;
        private readonly IRedisService _redisService;
        private readonly ILogger<PublishNotifyByBidv> _logger;
        ...
        public PublishNotifyByBidv(ILogger<PublishNotifyByBidv> logger,
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            while (!stoppingToken.IsCancellationRequested) {
                _logger.LogInformation("BankorderNotifyByBidv Worker running at: {time}", DateTimeOffset.Now);
                ...
                try
                {
                    foreach (var item in merchantConfigs)
                    {
                        ...
                        if (banks != null)
                        {
                            var bidvBankInfo = banks.Where(r => r.IsOpen & r.Type == PayMentTypeEnum.BidvBank).FirstOrDefault();
                            if (bidvBankInfo != null)
                            {
                                var bankOrderNotifysList = new List<BankOrderNotifyModel>();
                                while (true)
                                {
                                    var bankOrderNotifys = _redisService.GetBidvBankOrderNotifyMqPublish();
                                    ...
                                    if (bankOrderNotifysList.Count >= notifyCount || bankOrderNotifys == null)
                                    {
                                        ...
                                        var bankNotifies = bankOrderNotifysList.Where(r => paygroupInfo.Contains(r.PayMentId) && r.Money >= bidvBankInfo.Money).ToList();
                                        if (bankNotifies.Any())
                                        {
                                            ...
                                            _logger.LogInformation("商户：{merchantcode},bidv发送mq数据：{data}", item.MerchantCode, bankNotifies.ToJsonString());
                ...
                {
                    _logger.LogError("BankorderNotifyByBidv 队列错误：" + ex);
                }
