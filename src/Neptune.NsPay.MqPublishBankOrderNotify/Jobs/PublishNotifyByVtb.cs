...
{
    public class PublishNotifyByVtb : BackgroundService
    {
        private readonly ICapPublisher _capBus;
        private readonly IRedisService _redisService;
        private readonly ILogger<PublishNotifyByVtb> _logger;
        ...
        public PublishNotifyByVtb(ILogger<PublishNotifyByVtb> logger,
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            while (!stoppingToken.IsCancellationRequested) {
                _logger.LogInformation("PublishNotifyByVtb Worker running at: {time}", DateTimeOffset.Now);
                ...
                try
                {
                    foreach (var item in merchantConfigs)
                    {
                        ...
                        if (banks != null)
                        {
                            var vtbBankInfo = banks.Where(r => r.IsOpen & r.Type == PayMentTypeEnum.VietinBank).FirstOrDefault();
                            if (vtbBankInfo != null)
                            {
                                var bankOrderNotifysList = new List<BankOrderNotifyModel>();
                                while (true)
                                {
                                    var bankOrderNotifys = _redisService.GetVtbBankOrderNotifyMqPublish();
                                    ...
                                    if (bankOrderNotifysList.Count >= notifyCount || bankOrderNotifys == null)
                                    {
                                        ...
                                        var bankNotifies = bankOrderNotifysList.Where(r => paygroupInfo.Contains(r.PayMentId) && r.Money >= vtbBankInfo.Money).ToList();
                                        if (bankNotifies.Any())
                                        {
                                            ...
                                            _logger.LogInformation("商户：{merchantcode},Vtb发送mq数据：{data}", item.MerchantCode, bankNotifies.ToJsonString());
                ...
                {
                    _logger.LogError("PublishNotifyByVtb 队列错误：" + ex);
                }
