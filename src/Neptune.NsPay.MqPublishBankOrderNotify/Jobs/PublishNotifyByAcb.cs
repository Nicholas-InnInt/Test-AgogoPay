...
{
    public class PublishNotifyByAcb : BackgroundService
    {
        private readonly ICapPublisher _capBus;
        private readonly IRedisService _redisService;
        private readonly ILogger<PublishNotifyByAcb> _logger;
        ...
        public PublishNotifyByAcb(ILogger<PublishNotifyByAcb> logger,
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                _logger.LogInformation("PublishNotifyByAcb Worker running at: {time}", DateTimeOffset.Now);
                ...
                try
                {
                    foreach (var item in merchantConfigs)
                    {
                        ...
                        if (banks != null)
                        {
                            var acbBankInfo = banks.Where(r => r.IsOpen & r.Type == PayMentTypeEnum.ACBBank).FirstOrDefault();
                            if (acbBankInfo != null)
                            {
                                var bankOrderNotifysList = new List<BankOrderNotifyModel>();
                                while (true) 
                                {
                                    var bankOrderNotifys = _redisService.GetAcbBankOrderNotifyMqPublish();
                                    ...
                                    if (bankOrderNotifysList.Count >= notifyCount || bankOrderNotifys == null)
                                    {
                                        ...
                                        var bankNotifies = bankOrderNotifysList.Where(r => paygroupInfo.Contains(r.PayMentId) && r.Money >= acbBankInfo.Money).ToList();
                                        if (bankNotifies.Any())
                                        {
                                            ...
                                            _logger.LogInformation("商户：{merchantcode},acb发送mq数据：{data}", item.MerchantCode, bankNotifies.ToJsonString());
                                        }
                ...
                {
                    _logger.LogError("BankorderNotifyByAcb 队列错误：" + ex);
                }
