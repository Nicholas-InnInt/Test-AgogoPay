...
namespace Neptune.NsPay.Authorization.Users
{
    ...
    public class UserManager : AbpUserManager<Role, User>
    {
        ...
        public async Task<List<Merchant>> GetUserMerchantAsync(User user)
        {
            var userMerchants = await _abpUserMerchantRepository.GetAllListAsync(r => r.UserId == user.Id);
            List<Merchant> merchants = new List<Merchant>();
            foreach (var item in userMerchants)
            {
                var merchant = await _merchantRepository.FirstOrDefaultAsync(r => r.Id == item.MerchantId);
                if (merchant != null)
                {
            ...
            {
                merchants = await _merchantRepository.GetAllListAsync();
            }
            return merchants;
        }
